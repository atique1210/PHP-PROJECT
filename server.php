...
if (isset($_POST['reg_user'])) {
  ...
  if( !preg_match( '/[^A-Za-z0-9]+/', $password_1) || strlen( $password_1) < 6)
  {
     array_push($errors,"Invalid password!");
  }
  if( !preg_match( '/[^A-Za-z]+/', $firstname) || strlen( $firstname) < 0)
  {
...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    ...
    if (mysqli_num_rows($results) == 1) {
      if ($email == 'admin@admin.com' && $password == 'admin2412') // admin by default has fixed username and PASSWOD
       {
